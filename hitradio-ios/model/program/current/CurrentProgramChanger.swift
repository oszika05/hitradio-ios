...
class CurrentProgramChanger {
    ...
    private let defaultProgram: Program
    
    init(programs: [Program], observer: CurrentProgramObserver, defaultProgram: Program? = nil) {
    ...
    private func getCurrentProgram() -> Program? {
        let currentTime = Date()

        return programs.sorted { (a: Program, b: Program) in
            // sort by dates asc
    ...
    private func getNextProgram() -> Program? {

        // add 2 seconds to the time to avoid
        let currentTime = Date().addingTimeInterval(2.0)

        return self.fillGaps(programs: programs).sorted { (a: Program, b: Program) in
            // sort by dates asc
            return b.start.compare(a.start).rawValue > 0
        }.filter { program in
            // programs after now
    ...
    private func fillGaps(programs: [Program]) -> [Program] {
    ...
    private func setTimer(for program: Program) {
